...
import { bulkDeleteTrasactions } from "@/actions/accounts";
...
const TransactionTable = ({ transactions }) => {
    ...
    const {
        ...
        fn: deleteFn,
        data: deleted,
    }  =  useFetch(bulkDeleteTrasactions);
    ...
    const filteredAndSortedTransactions = useMemo(()=> {
        ...
        if( searchTerm ) {
            ...
            result = result.filter((trasaction) => 
               trasaction.description?.toLowerCase().includes(searchLower)
            );
        }
        ...
        if(recurringFilter) {
            result = result.filter((trasaction) => {
                if(recurringFilter === "recurring" ) return  trasaction.isRecurring;
    ...
    return <div className="space-y-4">
        ...
        {deleteLoading && ( <BarLoader className="mt-4" width={"100%"}  color="#7600bc"/>)}
        {/* Filter */}
                                ...
                                <TableCell>
                                    {transaction.isRecurring ? (
                                        <TooltipProvider>
                                            <Tooltip>
                                                <TooltipTrigger>
                                                    <Badge variant="outline" className="gap-1 bg-purlpe-100 text-purple-700  hover:bg-purple-200">
                                    ...
                                    <DropdownMenu>
                                        ...
                                        <DropdownMenuContent>
                                            <DropdownMenuLabel  className="cursor-pointer" onClick = {()=> 
                                                router.push(`/transaction/create?edit=${transaction.id}`)}>Edit
                                            </DropdownMenuLabel>
